...
class GodotAndroidPlugin(godot: Godot) : GodotPlugin(godot) {
    ...
    override fun getPluginSignals() = setOf(CONNECTION_STATE_SIGNAL, CONNECTED_SIGNAL, SIGNED_MESSAGE_SIGNAL)
    ...
    @UsedByGodot
    private fun setup(magic_key: String, network: String): Boolean {
        activity?.let { activity ->
            runOnUiThread {
                if (network == "polygon") {
                    val customNode = CustomNodeConfiguration("https://polygon-rpc.com/", "137")
                    GlobalContext.magic =
                        Magic(activity.applicationContext, magic_key, customNode)
                } else {
                    GlobalContext.magic = Magic(activity.applicationContext, magic_key)
                }
